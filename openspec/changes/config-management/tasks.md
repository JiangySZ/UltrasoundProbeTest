...
- [x] 2.1.1 创建ConfigManager类
  - 单例模式
  - 配置加载接口
- [x] 2.1.2 创建JsonLoader类
  - JSON文件读取
  - 错误处理
- [x] 2.1.3 实现ProbeConfig数据结构
- [x] 2.1.4 实现WaveformConfig数据结构
...
- [x] 2.2.3 定义thresholds字段
  - sensitivity, centerFrequency, bandwidth6dB, bandwidth20dB, pulseLength20dB
- [x] 2.2.4 创建示例配置文件 config/ProbeConfig.json

### 2.3 创建WaveformConfig.json schema

- [x] 2.3.1 定义waveform字段
  - name, type, txVoltage, frequency, cycles, points
- [x] 2.3.2 创建示例配置文件 config/WaveformConfig.json

### 2.4 实现配置文件验证功能

- [x] 2.4.1 创建Validator类
- [x] 2.4.2 实现必填字段验证
- [x] 2.4.3 实现数值范围验证
- [x] 2.4.4 实现阈值逻辑验证
- [x] 2.4.5 实现ValidationResult返回

### 2.5 实现TXT灵敏度列表导入

- [x] 2.5.1 创建SensitivityImporter类
- [x] 2.5.2 实现TXT文件解析
- [x] 2.5.3 实现每行一个数值格式
- [x] 2.5.4 实现数值校验
- [x] 2.5.5 创建示例TXT文件 config/SensitivityList.txt
